...
public class JwtTokenProviderTests
{
    ...
    [TestMethod("発行したトークンを検証できる(sub/unique_nameが取得できる)")]
    public void Issue_Then_Validate_Succeeds_And_ClaimsReadable()
    {
        // データを用意する
        var user = new User("alice", "pass001");
        // トークンを生成する
    ...
    {
        // トークンを生成する
        var user = new User("carol", "pass001");
        var token = _jwt.IssueAccessToken(user);
    ...
    public void GetPrincipalFromExpiredToken_Returns_Claims_When_Expired()
    {
        ...
        var user = new User("dave", "pass001");
