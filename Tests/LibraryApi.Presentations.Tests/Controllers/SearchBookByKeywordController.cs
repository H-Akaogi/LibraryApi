...
public class SearchBookByKeywordControllerTests
{
    ...
    [TestMethod("存在するキーワードの場合、ステータス200と図書リストを返す（蛍光 → 4件）")]
    public async Task Search_ShouldReturnOkWithBooks_WhenKeywordExists()
    {
        ...
        {
            _testContext?.WriteLine(book.ToString());
        }
    }
    ...
    {
        var result = await _controller!.Search("ミッケ");
        // result(IActionResult)をOkObjectResultに変換する
        var ok = result as OkObjectResult;
        // nullでないことを検証する
        Assert.IsNotNull(ok);
